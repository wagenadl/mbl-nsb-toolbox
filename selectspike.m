...
%----------------------------------------------------------------------
function phsc_loaddat(f, spk)
global phsc_data
f = double(f);

phsc_data{f}.ifn = 'data';
phsc_data{f}.src.spk.tms = spk.tms;
phsc_data{f}.src.spk.chs = 1+0*spk.tms;
phsc_data{f}.src.spk.hei = spk.amp;

phsc_process_spikes(f);

%----------------------------------------------------------------------
function phsc_process_spikes(f)
global phsc_data
f = double(f);

phsc_data{f}.loaded = 1;
phsc_data{f}.c = 1;
phsc_data{f}.T = max([max(phsc_data{f}.src.spk.tms) 1]);

phsc_data{f}.lower_thr = cell(1, 1);
phsc_data{f}.upper_thr = cell(1, 1);
if isfield(phsc_data{f}.src, 'chnames')
  phsc_data{f}.chnames = phsc_data{f}.src.chnames;
else
  phsc_data{f}.chnames = cell(1, 1);
end

dT = min(60, phsc_data{f}.T/2);

c=1;
phsc_data{f}.lower_thr{c} = [dT 10];
phsc_data{f}.upper_thr{c} = [dT 15];

delete(findobj(f, 'tag', 'channelselect'));

figure(f);

set(f, 'name', sprintf('Posthoc_SpikeClass: %s', phsc_data{f}.ifn));

setappdata(f, 'axlim0', [0 phsc_data{f}.T/60 -60 60]);
setappdata(f, 'axlim', [0 phsc_data{f}.T/60 -60 60]);

phsc_redraw(f, 1);


%----------------------------------------------------------------------
function phsc_redraw(f, graphtoo)
global phsc_data
f = double(f);

axes(findobj(f, 'tag', 'graph'));
a=getappdata(f, 'axlim');
axis(a);

if graphtoo
  c = phsc_data{f}.c;

  idx = find(phsc_data{f}.src.spk.chs==c);
  xx = phsc_data{f}.src.spk.tms(idx) / 60;
  yy = phsc_data{f}.src.spk.hei(idx);
  sigm=median(abs(yy(yy~=0)))/10;
  yy = sw_sig2log(yy/sigm);
  a=axis;
  idx=find(xx>=a(1) & xx<=a(2) & yy>=a(3) & yy<=a(4));
  [nn, xx, yy] = hist2(xx(idx), yy(idx), phsc_data{f}.axesw, phsc_data{f}.axesh);
  
  nn = gsmooth(gsmooth(nn, 2.5)', 1)';
  
  h = findobj(f, 'tag', 'trace');
...
a=axis;
c=phsc_data{f}.c;
xx = phsc_data{f}.lower_thr{c}(:, 1) / 60;
yy = sw_sig2log(phsc_data{f}.lower_thr{c}(:, 2));
h = findobj(gca, 'tag', 'lowerdots');
set(h, 'xdata', xx, 'ydata', yy);
h = findobj(gca, 'tag', 'lowerline');
set(h, 'xdata', [a(1); xx; a(2)], 'ydata', [yy(1); yy; yy(end)]);

xx = phsc_data{f}.upper_thr{c}(:, 1) / 60;
yy = sw_sig2log(phsc_data{f}.upper_thr{c}(:, 2));
...
iszoom = get(findobj(f, 'tag', 'zoom'), 'value');
...
switch tag
  ...
  otherwise
	...
	case {'alt', 'open'}
	  setappdata(f, 'axlim', getappdata(f, 'axlim0'));
	otherwise
	  xy=get(gca, 'currentpoint'); xy = xy(1, 1:2);
	  rbbox;
	  ...
	  if dd>5
	    setappdata(f, 'axlim', [sort([xy(1) xy1(1)]) sort([xy(2) xy1(2)])]);
	  else
	    setappdata(f, 'axlim', getappdata(f, 'axlim0'));
	  end
      end
...
if act
  xx = phsc_data{f}.upper_thr{c}(:, 1);
  yy = phsc_data{f}.upper_thr{c}(:, 2);
  nn = find(xx(2:end)<xx(1:end-1));
  xx(nn) = (xx(nn+1)+xx(nn))/2; xx(nn+1)=[];
  yy(nn) = (yy(nn+1)+yy(nn))/2; yy(nn+1)=[];
  phsc_data{f}.upper_thr{c} = [xx yy];
  
  xx = phsc_data{f}.lower_thr{c}(:, 1);
  yy = phsc_data{f}.lower_thr{c}(:, 2);
  nn = find(xx(2:end)<xx(1:end-1));
  xx(nn) = (xx(nn+1)+xx(nn))/2; xx(nn+1)=[];
  yy(nn) = (yy(nn+1)+yy(nn))/2; yy(nn+1)=[];
  phsc_data{f}.lower_thr{c} = [xx yy];
...
switch whch
  case 'lower'
    phsc_data{f}.lower_thr{c} = [xx*60 sw_log2sig(yy)];
  case 'upper'
    phsc_data{f}.upper_thr{c} = [xx*60 sw_log2sig(yy)];
end
...
global phsc_data
f = double(gcbf);
phsc_data{f}.c = get(gcbo, 'userdata');
...
%----------------------------------------------------------------------
function [gdspk, idx] = phsc_getdata(f)
global phsc_data
f = double(f);
...
c=1;
idx = find(phsc_data{f}.src.spk.chs==c);
tt = phsc_data{f}.src.spk.tms(idx);
yy = phsc_data{f}.src.spk.hei(idx);
sigm=median(abs(yy(yy~=0)))/10;
yy = yy/sigm;

x = phsc_data{f}.lower_thr{c}(:, 1);
y = phsc_data{f}.lower_thr{c}(:, 2);
[x, ord]=sort(x);
y = y(ord);
...
x = phsc_data{f}.upper_thr{c}(:, 1);
y = phsc_data{f}.upper_thr{c}(:, 2);
[x, ord]=sort(x);
y = y(ord);
...
gdspk.tms = phsc_data{f}.src.spk.tms(idx);
gdspk.amp = phsc_data{f}.src.spk.hei(idx);
